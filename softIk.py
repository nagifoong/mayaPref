...
def softIk(ctrl = "", ikHandle = ""):
    ...
    distanceSub = cmds.shadingNode("plusMinusAverage",name = "%s_softIk_distance_SUB"%pref,au=1)
    diffSub = cmds.shadingNode("plusMinusAverage",name = "%s_softIk_diff_SUB"%pref,au=1)
    negUc = cmds.shadingNode("unitConversion",name = "%s_softIk_diff_NEG"%pref,au=1)
    expNode = cmds.shadingNode("multiplyDivide",name = "%s_exp_diff_POW"%pref,au=1)
    revSub = cmds.shadingNode("plusMinusAverage",name = "%s_rev_exp_SUB"%pref,au=1)
    expMult = cmds.shadingNode("multiplyDivide",name = "%s_softIk_exp_MULT"%pref,au=1)
    finalAdd = cmds.shadingNode("plusMinusAverage",name = "%s_softIk_ADD"%pref,au=1)
    condNode = cmds.shadingNode("condition",name = "%s_softIk_COND"%pref,au=1)
    ...
    cmds.setAttr("%s.input1D[0]"%revSub,1)
    ...
    cmds.connectAttr("%s.outputX"%expNode,"%s.input1D[1]"%revSub)
    ...
    ##clean up
    utilGrp = cmds.group(name = "%s_softIk_utilGrp"%pref,em=1)
    softIkGrp = cmds.group(name = "%s_softIk_grp"%pref,em=1)
    newIkGrp = cmds.group(name = "%s_ikHandle_result_grp"%pref,em=1)
    
    cmds.xform(softIkGrp,t=(cmds.xform(softIk[0],q=1,ws=1,t=1)))
    cmds.xform(newIkGrp,t=(cmds.xform(ikMain,q=1,ws=1,t=1)))
    
    cmds.parent(locB,softIk[0],softIkGrp)
    cmds.parent(ikMain,newIkGrp)
    cmds.parent(softIkJnts[0],newIkGrp,softIkGrp,locA,dimNode.replace("Shape",""),utilGrp)
    
    cmds.pointConstraint(softIkJnts[-1],newIkGrp,mo=1)
    cmds.parentConstraint(ctrl,softIkGrp,mo=1)
    
    return [newIkGrp,utilGrp]
